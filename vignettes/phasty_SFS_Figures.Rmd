...
Consider first, the left figure of Figure 1. This displays the coefficients of for the site frequency spectrum for different unbiased estimators of the mutation rate $\theta$. The coefficients of the  H, L, Wattersons, pairwise and singleton estimators are given Section 1.1 of Asger [-@asgertemp2020]. The coefficients of the MVUE estimator are given by
\begin{eqnarray}
  \hat{\bc}=
...
txtVec <- c("Watterson","Singleton","Pairwise","H","L","MVUE")
...
plot(thetaVec,vrMVUE,type="l",lty=1,lwd=2,xlim=c(0,max(thetaVec)),
     xlab=bquote(theta),ylab="Variance for estimator",cex.lab=1.2,las=1)
...
ph_obj=phase_type(subintensity_matrix) #LNA - bruges denne?
ph_rew_obj=phase_type(subintensity_matrix, reward_mat = rew_mat) #Bruges denne?
```
...
```{r}
themean = sum(mean(ph_rew_obj)*bc) #the mean of the linear combination (should be 0)
...
```{r}
appvals=ApproxCDF(phi,H = 1e5,eta=0.0001,xlim=c(-0.5*res,1.5*res))
...
plot(xvals2,yvals,type="l",ylim=c(0,1),ylab="",xlab="",las=1)
...
svec = c(0.1,0.20,0.4,0.20,0.1)
simple_smoothe <- function(x,svec) {
  out = x
...
smoothed_yvals = simple_smoothe(yvals,svec)

plot(xvals2,smoothed_yvals,type="l",ylim=c(0,1),ylab="",xlab="",las=1)
lines(ecdfobj2,col="blue")
lines(xvals2,smoothed_yvals,lwd=2)
...
themean = sum(mean(ph_rew_obj)*bc) #the mean of the linear combination
bT = subintensity_matrix #bold T
bR = rew_mat #bold R
bS = bT - lambda*diag(rowSums(bR)) #bold S
bone = matrix(1,dim(bT)[1],1) #bold one = (1,1,...,1)^T
...
#Invert numerically
appvals=ApproxCDF(phi,H = 1e5,eta=0.0001,xlim=c(-1*res,2*res))
...
Figure 6 displays the point probabilities of  
the three integer--weighted estimators for $\theta$ ($\hat{\theta}_{\pi}, \hat{\theta}_H$ and $\hat{\theta}_L$ for $n=4$ and $n=6$. First, we consider the left sub-figure, i.e. $n=4$. The three estimators are all integer-weighted versions of the site frequency spectrum, i.e., of the form $\ba^\ast \bxi = a_1 \xi_1 + a_2 \xi_2 + \dots + a_{n-1} \xi_{n-1}$, where the $a_i$s are integers. As described in section 4.1 of Asger [-@asgertemp2020], these estimators have a $DPH$ distribution, whose subtransition matrix is constructed using the entries of ($\spadesuit$), so we first, calculate the latter:
...
m = dim(RMASS$RateM)[1] - 1

# Obtain subintensity matrix
subintensity_matrix = RMASS$RateM[1:m,1:m]
...
bc = res*bc
#compute characteristic function
themean = lambda*sum(mean(ph_rew_obj)*bc) #the mean of the linear combination
bT = subintensity_matrix #bold T
bR = rew_mat #bold R
bS = bT - lambda*diag(rowSums(bR)) #bold S
bone = matrix(1,dim(bT)[1],1) #bold one = (1,1,...,1)^T
...
#Invert numerically
appvals=ApproxCDF(phi,H = 1e5,eta=0.0001,xlim=c(-1.5*res,4*res))
...
lines(ecdf(ph_counts%*%xWatt-1),col="blue")

plot(xvals2,yvals,type="l",ylim=c(0,1),xlab="",ylab="")
lines(ecdfobj2,col="blue")
lines(xvals2,yvals,lwd=2)

lines(ecdf(ph_counts%*%xpair-1),col="blue")

ph = phase_type(ph_rew_obj$subint_mat)
watter=reward_phase_type(ph, rowSums(ph_rew_obj$reward_mat))
bM = solve( diag(dim(watter$subint_mat)[1])-(1/lambda)*watter$subint_mat)
bm = rowSums(diag(dim(bM)[1]) - bM)
ph$init_probs%*%(bM%^%0)%*%bm
ph$init_probs%*%(bM%^%1)%*%bm
ph$init_probs%*%(bM%^%2)%*%bm
ph$init_probs%*%(bM%^%3)%*%bm

table(ph_counts%*%xWatt-1)/R
...
# Possible values for Wattersons Theta
...
wx=-1+a1*(0:(vals-1))
lines(stepfun(wx,cumsum(c(0,out))),col="red")

#Last plot

RMASS = RateMAndStateSpace(n) #Todo - fix this, can be obtained from the objects above
m = dim(RMASS$RateM)[1] - 1
bC = RMASS$StSpM[1:m,1:(n-1)]

xpair2 = ( 1:(n-1) )*( (n-1):1 )
ba = xpair2 #This is the current a-vector
sizes_of_blocks = rep(0,m) #obtain the sizes of the blocks using formula XX
for(i in 1:m) {
  sizes_of_blocks[i]=max(ba*(bC[i,] > 0))
}
...
a1 = n*(n-1)/2
vals = 17
wx=-1+(1/a1)*(0:(length(probs2)-1))
plot(xvals2,yvals,type="l",ylim=c(0,1),xlab="",ylab="")
lines(stepfun(wx,cumsum(c(0,probs2))),col="red")
