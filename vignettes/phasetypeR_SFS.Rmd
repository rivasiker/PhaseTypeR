...
# Here we provide som auxillary functions which are needed below.
...
In this vignette, we recreate the figures from the paper.
For reproducibility of the simulations below, we first fix the seed of the random generator. We then load the \texttt{PhaseTypeR} package and auxiliary functions, which we will need below.
...
Consider first, the left figure of Figure 1. This figure displays the coefficients of the site frequency spectrum for different unbiased estimators of the mutation rate $\theta$. The coefficients of the  H, L, Wattersons, pairwise and singleton estimators are given Section 1.1 of Hobolth [-@asgertemp2020]. The coefficients of the BLUE estimator are given by
\begin{eqnarray}
  \hat{\bc}=
...
## Simulation and characteristic function - Figure 2
...
\end{align*}
where $\bT,\bA$ being respectively the subintensity matrix and reward matrix of the block counting process, $\be = (1,1,\dots,1)^\ast$ and $\bS = \bT - \lambda \Delta \bA \be$.
...
```{r,include=FALSE}
par(cex.axis=2, cex.lab=1.5, cex.main=1.5, cex.sub=1.3)
``` 
...
wxt <- -10+a1*(0:(len-1)) # Possible values for Wattersons Theta
