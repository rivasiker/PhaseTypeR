...
output: 
  html_document:
    toc: true
vignette: >
  %\VignetteIndexEntry{Using phasty for population genetics-cop}
...
This function will define an object of class `cont_phase_type` with  which will enable to use the `phasty` specific functions.
Consider $n = 4$, then the $T_{MRCA}$ can be reach with
...
There are a number of methods associated with the class `cont_phase_type`. For example, the mean and variance for $T_{MRCA}$ can be computed using: 
...
Other coalescent models can be define as phase-type distributed, for the example, we provide the function to get the phase-type distribution for the beta-coalescent model for a given n and $\alpha$ see TODO REF BETA 
```{r, warning = FALSE}
...
x <- seq(0, 8, 0.1)
y <- dphtype(x, T_mrca_4)
data <- data.frame(X1 = x, X2 = y)

x <- matrix(seq(0, 10, 0.1))
mrca <- data.frame(cbind(x, dphtype(x, T_mrca_4)))
total <- data.frame(cbind(x, dphtype(x, T_total_4)))
mrca['curve'] = 'MRCA'
total['curve'] = 'TOTAL'
data <- rbind(mrca, total)

p1 <- ggplot(data, aes(x = X1, y = X2, fill = curve)) + geom_area(alpha = 0.6, position = "identity") +  geom_point(size = 0.4) + theme_bw() + labs(x = "Time to absorbing state", y = "Density") + scale_fill_manual(values=c("#ff9900", "#408cce")) + theme(legend.title=element_blank(), text = element_text(size = 12))
...
x <- matrix(seq(0, 0.99, 0.01))
mrca <- data.frame(cbind(x, qphtype(x, T_mrca_4)))
total <- data.frame(cbind(x, qphtype(x, T_total_4)))
mrca['curve'] = 'MRCA'
total['curve'] = 'TOTAL'
data <- rbind(mrca, total)

p1 <- ggplot(data, aes(x = X1, y = X2, color = curve)) + geom_line(alpha = 0.6, position = "identity") +  geom_point(size = 0.6) + theme_bw() + labs(x = "Quantile" , y = "Time to absorbing state") + scale_color_manual(values=c("#ff9900", "#408cce")) + theme(legend.title=element_blank(), text = element_text(size = 12))
...
x <- matrix(seq(0, 10, 0.1))
mrca <- data.frame(cbind(x, pphtype(x, T_mrca_4)))
total <- data.frame(cbind(x, pphtype(x, T_total_4)))
mrca['curve'] = 'MRCA'
total['curve'] = 'TOTAL'
data <- rbind(mrca, total)

p1 <- ggplot(data, aes(x = X1, y = X2, color = curve)) + geom_line(alpha = 0.6, position = "identity") +  geom_point(size = 0.6) + theme_bw() + labs(x = "Time to absorbing state" , y = "Cumulated probability") + scale_color_manual(values=c("#ff9900", "#408cce")) + theme(legend.title=element_blank(), text = element_text(size = 12))
...
set.seed(0)
x1 <- rphtype(10000, T_mrca_4)
set.seed(0)
x2 <- rphtype(10000, T_total_4)
mrca <- data.frame(x = x1)
total <- data.frame(x = x2)
mrca['curve'] = 'MRCA'
total['curve'] = 'TOTAL'
data <- rbind(mrca, total)

p1 <- ggplot(data, aes(x = x, fill = curve)) + geom_histogram(position="identity", alpha = 0.5, binwidth = 0.2, color = 'grey10') + theme_bw() + labs(x = "Time to absorbing state" , y = "Cumulated probability") + scale_fill_manual(values=c("#ff9900", "#408cce")) + theme(legend.title=element_blank(), text = element_text(size = 12)) + xlim(c(0, 10))
print(p1)
```
...
The discrete phase-type representation of $S_{total}$ can be built from the continuous phase-type representation of the total branch length. Folllowing **theorem 3.5** in Hobolth et al. [-@Hobolth2019], the total branch length can be discretized if the mutation parameter $\theta$ is supplied. 
...
```
**Note** that the function ```block_couting_process``` to get the corresponding block counting process for the n-coalescent model for any $n$ is provided at the end of this document. Therefore ```sub_itons_4 <- block_counting_process(4)```
...
Note that we need to substract 1 from the mean to retrieve the true value. This is because of the way discrete phase-type distributions are defined (see [@Bladt2017] and [@Hobolth2019] for details).
