...
The reward-transformation is a way to give more weights to some states than other. So the first state of the subintensity matrix should have a rate of $n$, because before any coalescent event there are $n$ lineages, then the second state a rate of $n-1$ and so on.
...
We can define a continuous phase-type representation of $Y_i$ with the `phase_type` generator function:

```{r}

Y_i_phase_type <- phase_type(subint_mat = subint_itons_4, 
