...
=======
Instead of using a MJP (also called continuous Markov chain), it is also possible to consider a **Markov chain (MC)**. Let's consider $\tau \sim DPH(\boldsymbol{T}, \boldsymbol{\pi})$, where $\tau \in \mathbb{N}$. $\tau$ will represent the number of states visited before entering the absorbing state. For that reason, the sub-intensity matrix $\boldsymbol{T}$ will not contain rates, but rather the probability of jumping from one state to another. 
>>>>>>> fe895f7084b5e98b50b8c54af95ada9242952642
