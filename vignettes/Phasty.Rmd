...
The package ```phasty``` handle and manipulate the continuous and discrete phase-type distributions and their multivariate extensions. 

The package is not oriented for a specific application, but our initial interest in phase-type distributions is from applications in population genetics. 

# 2. The phase-type S3 class objects

The functions provided by ```phasty``` require objects of class ```disc_phase_type```, ```cont_phase_type```, ```mult_disc_phase_type``` or ```mult_cont_phase_type```, which can be obtained using the generator functions ```PH()```, ```DPH()```, ```MPH()``` and ```MDPH()```, respectively. 
...
An object of one of the phase-type classes always has a sub-intensity matrix, an initial probability vector, a defect (the probability of starting directly in the absorbing state, which can happen when using reward transformations (see Sectio~4)), and the type of class. 
...
Let $\{X_t\}_{t\ge0}$ be a **Markov jump process** with $p$ transient states and one absorbing state. If $\tau$ is defined as the time when $\{X_t\}_{t\ge0}$ reaches the absorbing state, then $\tau$ follows a continuous phase-type distribution $\tau \sim PH(\boldsymbol{T}, \boldsymbol{\pi})$, where $\boldsymbol{T}$ is the sub-intensity matrix of the corresponding Markov jump process, and $\boldsymbol{\pi}$ is the vector of initial probabilities.
...
Given the formulas from **Theorem 3.1.16** and **Corollary 3.1.18** [@Bladt2017 page p.135], the mean of a variable following a continuous phase-type distribution is given by

$$
\mathbb{E} (\tau) = \boldsymbol{\pi U e} = \boldsymbol{\pi} (-\boldsymbol{T})^{-1} \boldsymbol{e},
$$
and from **Corollary 3.1.18** [@Bladt2017 page p.135], the expectation of $\tau^n$ is given by
...
From this formula we find the variance of $\tau$ to be
...
**The probability density function** (PDF) of a continuous phase-type distribution is defined in **Theorem 3.1.7** [@Bladt2017 page p.~131]

$$
f(u) = \boldsymbol{\pi}e^{\boldsymbol{T}u}\boldsymbol{t}, \quad u\ge0,
$$
where $\boldsymbol{t} = -\boldsymbol{T}\boldsymbol{e}$ is the exit rate of the sub-intensity matrix (the vector of rates from state $i$ to the absorbing state). The **cumulative distribution function** (CDF) is

$$ 
F(u) = 1-\boldsymbol{\pi}e^{\boldsymbol{T}u}\boldsymbol{e}, \quad u\ge0.
$$

We calculate the PDF and the CDF with `dphtype()` and `pphtype()`, respectively,
...
{plot(x, cdf, xlab = "Time to absorption", ylab = "PDF and CDF", col = "orange", 
...
$$
 ...
 \mathbb{E} (\tau(\tau-1))=
   2\boldsymbol{\pi}\boldsymbol{T}(\boldsymbol{I}-\boldsymbol{T})^{-2}\boldsymbol{e}, \\
 \mathbb{V} (\tau) = \mathbb{E} (\tau(\tau-1)) + \mathbb{E} (\tau) - \mathbb{E} (\tau)^2.
$$

In ```phasty``` the mean and variance of a discrete phase-type distribution can be obtained using the generic functions `mean()` and `var()`:
...
**Theorem 1.2.58 and 1.2.59** in [@Bladt2017 page p.30] shows that the probability distribution of a discrete phase-type distributed variable is
...
x <- seq(0.05, 0.95, 0.01)
{plot(x, qphtype(x, dph), col = "orange", ylab = "Time to absorption", xlab = "Quantile",xlim=c(0,1),ylim=c(0,10))
title('Quantile function')}
```
...
A reward transformation allows to give a non-negative weights to each state. 

### 3.3.1 Strictly positive rewards for continuous phase-type distribution
...
which corresponds to the weighted time before absorption (or the reward earned until absorption). From **theorem 3.1.33** [@Bladt2017] it follows that this new variable also follows a continuous phase-type distributed 

$$
Y \sim PH(\boldsymbol{T}^\star, \boldsymbol{\pi}^\star).
$$

Deriving $\boldsymbol{T}^\star$ and $\boldsymbol{\pi}^\star$ is rather simple if the given rewards only contain positive values, in that case, the rates are divided by the corresponding rewards ($\boldsymbol{T}^\star$ = $\boldsymbol{Tr}$) and the initial probabilities remain the same ($\boldsymbol{\pi}^\star$ = $\boldsymbol{\pi}$).
...
{plot(x, cdf, xlab = "Time to absorption", ylab = "", col = "orange", type = 'l',ylim=c(0,1))
...
### 3.3.2. Strictly positive rewards for discrete phase-type distribution
...
### 3.3.3. Non-negative reward transformation for continuous phase-type distribution

To perform a reward transformation with one or more rewards of zero, it is necessary to transform the sub-intensity matrix and initial probabilities by removing the zero rewarded states and then apply the positive rewards on the remaining states. 

**Theorem 3.1.33** [@Bladt2017 page p.147] gives the non-negative reward-transformed random variable of a phase-type distribution. 


Define the transition matrix $\boldsymbol{P}$ by 
...
### 3.3.4. Non-negative integer valued reward transformation for discrete phase-type distribution
