...
{plot(x, pdf, xlab = "Time to absorption", ylab = "pdf", col = "orange", type = 'l',ylim=c(0,1))
...
title('Distribution functions before and after reward transformation')
legend("topleft", bty="n", legend=c('Original DPH', 'Reward transformed DPH'),
...
A non-negative reward transformation of a phase-type distribution is also phase-type distributed (posssibe with a defect). **Theorem 3.1.33** [@Bladt2017 page p.147] gives the non-negative reward-transformed phase-type distribution. 
...
### 5.1.1 The variance-covariance of a multivariate phase-type distribution
...
The variance-covariance matrix is computed in ```phasty``` using ```var```
...
The discrete multivariate phase-type (MDPH) distribution is constructed similar to the continuous multivariate phase-type distribution, but in discrete time. Details are available in **Section 5.2** [@Navarro2019 page p.79]. We write  $\boldsymbol{Y} = (Y_1,\  Y_2,\  ...\,,\  Y_n)$ for the MDPH-distributed vector where $Y_i$ corresponds to
...
### 5.2.1 The variance-covariance of a discrete multivariate phase-type distribution
...
In ```phasty``` the variance-covariance matrix is computed using ```var()```. In this case we get
...
Comments regarding the last stuff:\\
I think it is a good idea to have the possibility of a full simulation of the underlying absorbing Markov chain. Perhpas call it rAbsMJP for 'random draw from an absorbing Markov Jump process' in the continuous case and rAbsDMC' for 'random draw from an absorbing discrete Markov chain'. An option in the rPH and rDPH should perhaps be to use this for simulating instead of the current procedure?
