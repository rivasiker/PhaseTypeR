...
output:
  ...
  pdf_document:
    toc: yes
vignette: |
  %\VignetteIndexEntry{Phasty} %\VignetteEncoding{UTF-8} %\VignetteEngine{knitr::rmarkdown}
...
Getting $\boldsymbol{T}^\star$ and $\boldsymbol{\pi}^\star$ is rather simple if the given rewards only contains positive values, in that case, the rate will be divided by the corresponding rewards ($\boldsymbol{T}^\star$ = $\boldsymbol{Tr}$) and the initial probabilities will not be modify ($\boldsymbol{\pi}^\star$ = $\boldsymbol{\pi}$).
