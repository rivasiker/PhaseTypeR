...
Let $\tau \sim PH(\boldsymbol{T}, \boldsymbol{\pi})$ and let $\{X_t\}_{t \ge 0}$ denote the underlying Markov jump process. Furthermore, let the reward vector be $\boldsymbol{r} = (r_1,\ r_2,\ ...\,,\ r_n) \in \mathbb{R}^p_+$.
...
which corresponds to the weighted time before absorption (or the reward earned until absorption). From **Theorem 3.1.33** [@Bladt2017] it follows that this new variable is also continuous phase-type distributed 

$$
Y \sim PH(\boldsymbol{T}^\star, \boldsymbol{\pi}^\star).
$$

Deriving $\boldsymbol{T}^\star$ and $\boldsymbol{\pi}^\star$ is rather simple if the rewards only contain positive values. In that case the rates are divided by the corresponding rewards ($\boldsymbol{T}^\star$ = $\triangle(\boldsymbol{1/r})\boldsymbol{T}$) and the initial probabilities remain the same ($\boldsymbol{\pi}^\star$ = $\boldsymbol{\pi}$).
...
Let $\tau \sim DPH(\boldsymbol{T}, \boldsymbol{\pi})$ and let $\{X_n\}_{n \ge 0}$ be the underlying Markov chain, and the reward vector  $\boldsymbol{r} = (r_1,\ r_2,\ ...\,,\ r_p) \in \mathbb{N}$. As for the continuous phase-type:
...
### 4.5. The variable reward in discrete phase-type
...
# 5. The multivariate phase-type distribution

## 5.1. The continuous multivariate phase-type
...
Belongs in Section 4 on reward transformations
Lars ToDo?
