...
Taking the previous example and applying a reward vector $\boldsymbol{r} = (1,\ 2,\ 3)$, the continuous phase-type distribution is reward-transformed in `phasty` using `reward_phase_type()`:
...
pdf <- dPH(x,ph)
rwd.trns.pdf <- dPH(x,rph)

{plot(x, pdf, xlab = "Time to absorption", ylab = "", col = "orange", type = 'l',ylim=c(0,1))
lines(x, rwd.trns.pdf, col = "blue")
title('Distribution functions before and after reward transformation')
legend("topleft", legend=c('Original Phase-type', 'Reward transformed Phase-type'),
