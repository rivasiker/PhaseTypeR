...
test_that(
  'test of all the error and warnings in continuous phase-type function', {
    ...
    expect_warning(try_merging(subint_mat = matrix(c(-1.5, 0, 0,
                                                         1.5, -1, 0,
                                                         0, 1, -0.5), ncol = 3)),
    ...
    expect_error(try_merging(init_probs = c(1, 0, 0)),
                 'Unable to construct')

    # [E] Negative values in the init
    expect_error(try_merging(subint_mat = matrix(c(-1.5, 0, 0,
                                                       1.5, -1, 0,
                                                       0, 1, -0.5), ncol = 3),
                                 init_probs = c(-1, 2, 0)),
    ...
    expect_error(try_merging(subint_mat = matrix(c(0, 0, 0,
                                                       0, -1, 0,
                                                       0, 1, -0.5), ncol = 3),
                                 init_probs = c(1, 0, 0)),
                 'diagonal')
...
test_that(
  'test of all the error and warnings in discrete phase-type function', {
    ...
    # [W] No init_probs provided
    expect_warning(try_merging(subint_mat = matrix(c(0.4, 0, 0,
                                                         0.24, 0.4, 0,
                                                         0.12, 0.2, 0.5), ncol = 3)),
                   'The initial probability vector is automatically generated.')
    ...
    # [E] Negative values in the init
    expect_error(try_merging(subint_mat = matrix(c(0.4, 0, 0,
                                                       0.24, 0.4, 0,
                                                       0.12, 0.2, 0.5), ncol = 3),
                                 init_probs = c(-1, 2, 0)),
                 'Each initial probability should be between 0 and 1')
    ...
    expect_error(try_merging(subint_mat = matrix(c(2, 0, 0,
                                                       0.24, 0.4, 0,
                                                       0.12, 0.2, 0.5), ncol = 3),
                                 init_probs = c(1, 0, 0)),
                 'rowsums')
...
test_that(
  'Test of the mean and variance for discrete phase-type.',{
    test1 = try_merging(subint_mat = matrix(c(0.4, 0, 0,
                                                  0.24, 0.4, 0,
                                                  0.12, 0.2, 0.5), ncol = 3))
