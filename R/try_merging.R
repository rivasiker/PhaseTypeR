...
#'
#' Description of the class \code{cont_phase_type}, which represents continuous
#' phase-type distributions.
#'
#' \code{cont_phase_type} is the generator function for the continuous
#' phase-type distribution class of the same name, which inherits from
#' \code{list}.
#' The class is generated by supplying a sub-intensity matrix and an optional
#' initial probability vector.
#' If the initial probabilities are not specified, then they are automatically
#' generated as the first state having a probability of 1 and the rest
#' a probability of 0.
#'
...
phase_type <- function(subint_mat = NULL, init_probs = NULL,
  ...
  if (is.matrix(reward_mat)){
    ...
    if (ncol(reward_mat) != length(init_probs)){
  ...
  if (sum(rowSums(subint_mat) > 0) == 0) {
    if (length(which(diag(subint_mat >= 0))) > 0){
  ...
  #############
    ...
    stop('This matrix is not valid for either discrete or continuous phase-type')
  }
...
#' \code{disc_phase_type} and \code{mult_phase_type} classes respectively.
#'
...
#' }
#'
#' @param obj a \code{cont_phase_type}, \code{disc_phase_type} or
#'  \code{mult_phase_type}
...
#' @describeIn cont_phase_type variance of the continuous phase-type
#' distribution.
#'
#' @export
...
#' @describeIn cont_phase_type pretty summary of the continuous phase-type
#' distribution.
#'
#' @export
...
#' @describeIn disc_phase_type pretty summary of the discrete phase-type
#' distribution.
#'
#' @export
...
#' @describeIn mult_phase_type if an integer is provided in \code{v},
#' then the mean of the variable with the specified index in the
...
#' @describeIn mult_phase_type if an integer is provided in \code{v},
#' then the variance of the variable with the specified index in the
#' reward matrix is returned.
#' If instead a vector of length 2 is provided, then the covariance of
#' the variables defined by those indices will be returned.
#' In case of not specifying \code{v}, the variance-covariance matrix between
#' all the variables defined by the sub-intensity matrix are returned (default).
#'
#' @export
