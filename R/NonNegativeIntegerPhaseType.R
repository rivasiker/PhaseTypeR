...
kMx <- 50
main.txt <- paste("Integer-weighted SFS distributions for n = ",
                  n, "and theta = ", tht)
plot((0:kMx) / (n * (n - 1) / 2), PrbP[1:(kMx + 1)],
     type = "l", lty = "dotted", xlim = c(0, 2),
     xlab = "Weighted SFS", ylab = "Probability",
     main = main.txt, cex.main = 1.2,cex.lab = 1.2, lwd = 2)

points(0:kMx/(n*(n-1)/2), PrbP[1:(kMx+1)], pch = 19, cex = 0.8)
points(0:kMx/(n*(n-1)/2), PrbH[1:(kMx+1)], type = "l", lty = "solid", lwd = 2)
points(0:kMx/(n*(n-1)/2), PrbH[1:(kMx+1)], pch = 19, cex = 0.8)
points(0:kMx/(n-1), PrbL[1:(kMx+1)], type = "l", lty = "dashed", lwd = 2)
points(0:kMx/(n-1), PrbL[1:(kMx+1)], pch = 19, cex = 0.8)
...
cvmat <- CovMatMPH(initVec = c(1, rep(0, nSt - 2)), subIntMat = subIntMat,
                   rewardMat = rewardMat)
